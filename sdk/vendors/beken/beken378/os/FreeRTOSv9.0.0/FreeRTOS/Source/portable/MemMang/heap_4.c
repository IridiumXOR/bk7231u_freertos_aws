...
size_t xTotalHeapSize = configTOTAL_HEAP_SIZE;
...
#if defined(CFG_DYNAMIC_HEAP)
    /* Ensure the heap starts on a correctly aligned boundary. */
    uxAddress = ( size_t ) &_empty_ram;
    xTotalHeapSize = ( size_t ) &_stack_unused;
    ...
    xTotalHeapSize &= ~( ( size_t ) portBYTE_ALIGNMENT_MASK );
    xTotalHeapSize -= uxAddress;
#else
	/* Ensure the heap starts on a correctly aligned boundary. */
...
{
#if OSMALLOC_STATISTICAL
    BlockLink_t *pxIterator = (BlockLink_t *)((u8*)pv - xHeapStructSize);
