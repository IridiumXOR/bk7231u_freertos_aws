...
extern BaseType_t xTaskIsTaskFinished( xTaskHandle xTask );
...
OSStatus rtos_print_stack(beken_thread_t *thread)
{
    ...
    vTaskGetInfo((TaskHandle_t)thread, &xTaskStatus, pdFALSE, eInvalid);
    ...
    {
        int index;
        os_printf("addr2line.exe -e aws_tests.axf -a -f");
...
OSStatus rtos_print_stack_by_name(char* name)
{
    TaskStatus_t *pxTaskStatusArray;
    unsigned portBASE_TYPE uxCurrentNumberOfTasks = uxTaskGetNumberOfTasks();
    volatile UBaseType_t uxArraySize, x;
    ...
    if (!strncmp(name, "all_tasks", sizeof("all_tasks")))
    {
        for ( x = 0; x < uxArraySize; x++ )
        {
            rtos_print_stack((beken_thread_t *)pxTaskStatusArray[x].xHandle);
        }
    }
    else
    {
        for ( x = 0; x < uxArraySize; x++ )
        {
            if (strcmp(name, pxTaskStatusArray[x].pcTaskName))
            {
                continue;
            }
            rtos_print_stack((beken_thread_t *)pxTaskStatusArray[x].xHandle);
