...
static void taskstack_Command( char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv )
{
    if (argc < 2)
    {
    ...
    {
        extern OSStatus rtos_print_stack_by_name(char* name);
        rtos_print_stack_by_name(argv[1]);
    }
}
#endif
...
OSStatus ping(struct in_addr *ina, uint32_t times, size_t size, uint32_t interval)
{
    int s, ttl, recv_len;
    struct timeval timeout = { PING_RCV_TIMEO /1000, PING_RCV_TIMEO % 1000 };
    ip_addr_t target_addr;
    uint32_t send_times;
    beken_time_t recv_start_tick;
    ...
    {
        os_printf("ping: unknown host 0x%x\n", ina->s_addr);
        return kGeneralErr;
    }
    /* new a socket */
    if ((s = lwip_socket(AF_INET, SOCK_RAW, IP_PROTO_ICMP)) < 0)
    {
        os_printf("ping: create socket failed\n");
        return kGeneralErr;
    }
    ...
    while (1)
    {
        ...
        {
            recv_start_tick = rtos_get_time();
            if ((recv_len = ping_recv(s, &ttl)) >= 0)
            {
                os_printf("%d bytes from %s icmp_seq=%d ttl=%d time=%d ticks\n", recv_len, inet_ntoa(*ina), send_times,
                        ttl, rtos_get_time() - recv_start_tick);
            }
            else
            {
                os_printf("From %s icmp_seq=%d timeout\n", inet_ntoa(*ina), send_times);
            }
        }
        else
        {
            os_printf("Send %s - error\n", inet_ntoa(*ina));
        }
        ...
        rtos_delay_milliseconds(interval); /* take a delay */
    }
...
void ping_Command(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
    ...
    if (argc < 2)
    {
        os_printf("usage: ping <ip|domain>\n");
        return;
    }
...
static const struct cli_command built_ins[] =
{
    ...
    {"taskstack", "taskstack [name] print thread stack by name", taskstack_Command},
