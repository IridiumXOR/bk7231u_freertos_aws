...
typedef struct _item_header_t {
    ...
    uint16_t    hash;           /* the hash code of key, for search */
...
typedef struct _kv_mgr_t {
    ...
    beken_semaphore_t gc_sem;
    beken_mutex_t kv_mutex;
...
static uint16_t utils_hash(uint8_t *buf, uint16_t length)
{
    uint32_t hash = 0;
    ...
    for (; buf < end; buf++)
    {
        ...
        hash = *buf + (hash << 7) + (hash << 1) + hash; /* the seed could be 13, 131, 13131 ... */
#else
        hash = *buf + hash * 131; /* the seed could be 13, 131, 13131 ... */
#endif
    }
...
static int raw_read(uint32_t offset, void *buf, size_t nbytes)
{
    ...
    return bk_flash_read(KV_PTN, &offset, buf, nbytes);
#else
    flash_read((unsigned char *)buf, nbytes, BK_KV_ADDRESS + offset);
#endif
    return RES_OK;
}
...
static int raw_write(uint32_t offset, const void *buf, size_t nbytes)
{
    ...
    bk_flash_write(KV_PTN, &offset, buf, nbytes);
    //bk_flash_enable_security(FLASH_UNPROTECT_LAST_BLOCK);
#else
    flash_write((unsigned char *)buf, nbytes, BK_KV_ADDRESS + offset);
#endif
    return RES_OK;
}
...
static int raw_erase(uint32_t offset, uint32_t size)
{
#if defined(BK_FLASH_PARTITION)
    //bk_flash_enable_security(FLASH_PROTECT_NONE);
    bk_flash_erase(KV_PTN, offset, size);
    //bk_flash_enable_security(FLASH_UNPROTECT_LAST_BLOCK);
#else
    ...
    offset = offset & BLK_OFF_MASK;

    addr = BK_KV_ADDRESS + offset;
    ...
    do{
        ...
        rtos_delay_milliseconds(5); // delay 5 ms every 4K
...
static void trigger_gc(void)
{
    ...
    rtos_create_thread(&thread, BEKEN_DEFAULT_WORKER_PRIORITY, "kv-gc", aos_kv_gc, KV_GC_STACK_SIZE, NULL);
}
...
static int kv_item_del(kv_item_t *item, int mode)
{
    ...
    if (mode == KV_SELF_REMOVE) {
        ...
        char *origin_key = (char *)os_malloc(hdr.key_len);
        if (!origin_key) {
            return RES_MALLOC_FAILED;
        }
        char *new_key = (char *)os_malloc(hdr.key_len);
        if (!new_key) {
            os_free(origin_key);
            return RES_MALLOC_FAILED;
        }
...
typedef int (*item_func)(kv_item_t *item, const char *key, uint16_t hash);

static int __item_recovery_cb(kv_item_t *item, const char *key, uint16_t hash)
{
    char *p = (char *)os_malloc(item->len);
    if (!p) {
        return RES_MALLOC_FAILED;
    }
...
static int __item_find_cb(kv_item_t *item, const char *key, uint16_t hash)
{   
    if (item->hdr.key_len != strlen(key)) {
        return RES_CONT;
    }
    
    if(hash != item->hdr.hash)
    {
        return RES_CONT;
    }

    item->store = (char *)os_malloc(item->hdr.key_len + item->hdr.val_len);
    if (!item->store) {
        return RES_MALLOC_FAILED;
    }
...
static int __item_gc_cb(kv_item_t *item, const char *key, uint16_t hash)
{
    char *p;
    int ret;
    uint16_t len;
...
static kv_item_t *kv_item_traverse(item_func func, uint8_t blk_index, const char *key)
{
    ...
    hash = utils_hash((uint8_t *)key, strlen(key));

    do {
        item = (kv_item_t *)os_malloc(sizeof(kv_item_t));
        if (!item) {
            return NULL;
        }
...
static int kv_item_store(const char *key, const void *val, int len, uint16_t origin_off)
{
    ...
    store.p = (char *)os_malloc(store.len);
    if (!store.p) {
        return RES_MALLOC_FAILED;
    }
    ...
    hdr.hash = utils_hash((uint8_t *)p, hdr.key_len);
...
void aos_kv_gc(void *arg)
{
    ...
    if (rtos_lock_mutex(&(g_kv_mgr.kv_mutex)) != 0) {
        goto exit;
    }
...
exit:
    ...
    rtos_unlock_mutex(&(g_kv_mgr.kv_mutex));
    if (g_kv_mgr.gc_waiter > 0) {
        rtos_set_semaphore(&(g_kv_mgr.gc_sem));
    }

    rtos_delete_thread(0);
}
...
int aos_kv_del(const char *key)
{
    kv_item_t *item;
    int ret;
    if ((ret = rtos_lock_mutex(&(g_kv_mgr.kv_mutex))) != RES_OK) {
        return ret;
    }
...
int aos_kv_set(const char *key, const void *val, int len, int sync)
{
    ...
    if (g_kv_mgr.gc_triggered) {
        (g_kv_mgr.gc_waiter)++;
        rtos_get_semaphore(&(g_kv_mgr.gc_sem), BEKEN_WAIT_FOREVER);
    }
...
/* CLI Support */
#if 1
static int __item_print_cb(kv_item_t *item, const char *key, uint16_t hash)
{
    char *p_key = (char *)os_malloc(item->hdr.key_len + 1);
    if (!p_key) {
        return RES_MALLOC_FAILED;
    }
    ...
    char *p_val = (char *)os_malloc(item->hdr.val_len + 1);
    if (!p_val) {
        os_free(p_key);
        return RES_MALLOC_FAILED;
    }
...
static void handle_kv_cmd(char *pwbuf, int blen, int argc, char **argv)
{
    ...
    if (strcmp(rtype, "set") == 0) {
        ...
        if (ret != 0) {
            os_printf("cli set kv failed\r\n");
        }
    } else if (strcmp(rtype, "get") == 0) {
        if (argc != 3) {
            return ;
        }
        buffer = os_malloc(BLK_SIZE);
        if (!buffer) {
            os_printf("cli get kv failed\r\n");
            return;
        }
        ...
        if (ret != 0) {
            os_printf("cli: no paired kv\r\n");
        } else {
            os_printf("value is %s\r\n", buffer);
        }
...
int aos_kv_init(void)
{
    ...
    if (g_kv_mgr.kv_initialize) {
        os_printf("%s:%d already initialized\n", __FUNCTION__, __LINE__);
        return RES_OK;
    }

    if (BLK_NUMS <= KV_GC_RESERVED) {
        os_printf("%s:%d no enough blocks\n", __FUNCTION__, __LINE__);
        return -EINVAL;
    }

    memset(&g_kv_mgr, 0, sizeof(g_kv_mgr));
    if ((ret = rtos_init_mutex(&(g_kv_mgr.kv_mutex))) != 0) {
        os_printf("%s:%d create mutex failed\n", __FUNCTION__, __LINE__);
        return ret;
    }

    ret = cli_register_commands(&ncmd, 1);

    if ((ret = kv_init()) != RES_OK) {
        os_printf("%s:%d initialize failed\n", __FUNCTION__, __LINE__);
        return ret;
    }

    if ((ret = rtos_init_semaphore(&(g_kv_mgr.gc_sem), 10)) != RES_OK) {
        os_printf("%s:%d create semaphore failed\n", __FUNCTION__, __LINE__);
        return ret;
    }
...
void aos_kv_deinit(void)
{
    ...
    rtos_deinit_semaphore(&(g_kv_mgr.gc_sem));
    rtos_deinit_mutex(&(g_kv_mgr.kv_mutex));
}
